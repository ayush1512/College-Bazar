      ...
      <div className="max-w-screen-xl mx-auto py-20 lg:py-24">
        <div className="flex flex-col items-center lg:items-stretch lg:flex-row flex-wrap">
          ...
          {blogPosts.map((post, index) => (
            <div key={index} className="mt-24 lg:w-1/3">
              <div className="lg:mx-4 xl:mx-8 max-w-sm lg:max-w-xs">
                <div className="bg-cover bg-center h-80 lg:h-64 rounded" style={{ backgroundImage: `url(${post.imageSrc})` }}></div>
                <div className="mt-4 text-secondary-100 font-bold text-sm">{post.category}</div>
                <h4 className="mt-2 leading-relaxed font-bold text-lg">{post.title}</h4>
                <a href={post.url} className="inline-block mt-2 text-sm text-primary-500 font-bold cursor-pointer transition duration-300 border-b-2 border-transparent hover:border-primary-500">Read Post</a>
              </div>
            </div>
          ))}
        </div>
      </div>
      <img src="../../images/svg-decorator-blob-1.svg" className="-z-10 absolute bottom-0 right-0 w-48 h-48 transform translate-x-40 -translate-y-8 opacity-25" />
      <img src="../../images/svg-decorator-blob-3.svg" className="-z-10 absolute top-0 left-0 w-48 h-48 transform -translate-x-32 translate-y-full opacity-25" />
    </div>
  );
};